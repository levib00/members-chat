...
passport.use(
  new LocalStrategy(async (username: string, password: string, done: (arg0: null | unknown, arg1?: boolean | undefined, arg2?: { message: string; } | undefined) => any) => {
    try {
...
app.get('/log-out', (req: { logout: (arg0: (err: any) => any) => void; }, res: { redirect: (arg0: string) => any; }, next: (arg0: any) => any) => {
  req.logout((err) => {
    if (err) {
      return next(err);
    }
