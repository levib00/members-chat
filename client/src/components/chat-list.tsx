...
const ChatroomList = () => {
  ...
  const { data: user, error: userError } = useSWR('https://levib00-chatroom.adaptable.app/api/users/user', getFetcher);

  // Fetching chatroom data using SWR
  const { data: chatrooms } = useSWR('https://levib00-chatroom.adaptable.app/api/chatrooms/', getFetcher);
